# 목적: F005 유튜브 컨텐츠 제작 V3 (채팅 기반) 파이프라인 Pydantic 스키마 정의
import sys
...
class F005JobCreateRequest(BaseModel):
    """POST /api/f005/jobs 요청 바디 — 채팅 기반 유튜브 콘텐츠 작업 생성 파라미터.
...
class F005StageResponse(BaseModel):
    """stages 테이블 단건 응답 스키마 (F005 전용)."""
...
class F005ContentJobResponse(BaseModel):
    """content_jobs 테이블 단건 응답 스키마 (F005 전용) — 스테이지 목록 포함."""
    ...
    stages: list[F005StageResponse] = []

    model_config = {"from_attributes": True}


class F005ContentJobListResponse(BaseModel):
    """GET /api/f005/jobs 목록 응답 스키마 — cursor 기반 페이징 (F005 전용)."""

    # 작업 목록
    items: list[F005ContentJobResponse]
...
class F005StageRetryRequest(BaseModel):
    """POST /api/f005/jobs/{job_id}/stages/{stage_id}/retry 요청 바디 (F005 전용)."""

    # 재시도 시 입력 파라미터 override (없으면 기존 input_data 사용)
    override_params: Optional[dict[str, Any]] = None


class F005StageRejectRequest(BaseModel):
    """POST /api/f005/jobs/{job_id}/stages/{stage_id}/reject 요청 바디 (F005 전용)."""
...
class F005ApproveRequest(BaseModel):
    """POST /api/f005/jobs/{job_id}/approve 요청 바디 — 업로드 최종 승인 (F005 전용)."""
...
class F005ScriptRerunRequest(BaseModel):
    """POST /api/f005/jobs/{job_id}/rerun-from-tts 요청 바디 — 스크립트 수정 후 TTS부터 재생성 (F005 전용)."""
