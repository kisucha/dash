# 목적: F004 유튜브 컨텐츠 제작 V2 파이프라인 Pydantic 스키마 정의
import sys
...
class F004JobCreateRequest(BaseModel):
    """POST /api/f004/jobs 요청 바디 — 유튜브 콘텐츠 작업 생성 파라미터."""
...
class F004StageResponse(BaseModel):
    """stages 테이블 단건 응답 스키마 (F004 전용)."""
...
class F004ContentJobResponse(BaseModel):
    """content_jobs 테이블 단건 응답 스키마 (F004 전용) — 스테이지 목록 포함."""
    ...
    stages: list[F004StageResponse] = []

    model_config = {"from_attributes": True}


class F004ContentJobListResponse(BaseModel):
    """GET /api/f004/jobs 목록 응답 스키마 — cursor 기반 페이징 (F004 전용)."""

    # 작업 목록
    items: list[F004ContentJobResponse]
...
class F004StageRetryRequest(BaseModel):
    """POST /api/f004/jobs/{job_id}/stages/{stage_id}/retry 요청 바디 (F004 전용)."""

    # 재시도 시 입력 파라미터 override (없으면 기존 input_data 사용)
    override_params: Optional[dict[str, Any]] = None


class F004StageRejectRequest(BaseModel):
    """POST /api/f004/jobs/{job_id}/stages/{stage_id}/reject 요청 바디 (F004 전용)."""
...
class F004TopicSelectRequest(BaseModel):
    """POST /api/f004/jobs/{job_id}/topics/{topic_rank}/select 요청 바디 (F004 전용)."""

    # 선택한 주제 제목
    selected_topic_title: str = Field(..., min_length=1, description="선택한 주제 제목")


class F004ApproveRequest(BaseModel):
    """POST /api/f004/jobs/{job_id}/approve 요청 바디 — 업로드 최종 승인 (F004 전용)."""
...
class F004ScriptRerunRequest(BaseModel):
    """POST /api/f004/jobs/{job_id}/rerun-from-tts 요청 바디 — 스크립트 수정 후 TTS부터 재생성 (F004 전용)."""
