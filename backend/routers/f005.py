# 목적: /api/f005 엔드포인트 라우터 — F005 채팅 기반 콘텐츠 작업 CRUD API
...
from schemas.f005 import (
    F005ApproveRequest,
    F005ContentJobListResponse,
    F005ContentJobResponse,
    F005JobCreateRequest,
    F005ScriptRerunRequest,
    F005StageRejectRequest,
    F005StageResponse,
    F005StageRetryRequest,
)
...
router = APIRouter(prefix="/api/f005", tags=["f005"])


# ── content_jobs CRUD ─────────────────────────────────────────────

@router.post("/jobs", response_model=F005ContentJobResponse, status_code=201)
async def create_job(
    request: F005JobCreateRequest,
    db: aiosqlite.Connection = Depends(get_db),
) -> F005ContentJobResponse:
    """새 F005 콘텐츠 작업을 생성하고 오케스트레이터를 독립 프로세스로 실행한다."""
    job = await f005_service.create_job(db, request)
    return F005ContentJobResponse.model_validate(job)


@router.get("/jobs", response_model=F005ContentJobListResponse)
async def list_jobs(
    ...
    """F005 content_jobs 목록을 최신 순(id DESC)으로 반환한다. cursor 기반 페이징."""
    items, next_cursor, has_more = await f005_service.list_jobs(
        db, limit=limit, cursor=cursor, status=status
    )
    return F005ContentJobListResponse(
        items=[F005ContentJobResponse.model_validate(item) for item in items],
        next_cursor=next_cursor,
        has_more=has_more,
    )


@router.get("/jobs/{job_id}", response_model=F005ContentJobResponse)
async def get_job(
    job_id: int,
    db: aiosqlite.Connection = Depends(get_db),
) -> F005ContentJobResponse:
    """ID로 F005 content_job 단건을 조회한다. 스테이지 목록 포함."""
    job = await f005_service.get_job(db, job_id)
    if job is None:
...
async def delete_job(
    ...
    """F005 content_job과 연관 stages를 DB에서 완전 삭제한다."""
    deleted = await f005_service.delete_job(db, job_id)
    if not deleted:
        raise HTTPException(status_code=404, detail=f"Job {job_id} 를 찾을 수 없습니다.")
...
@router.get("/jobs/{job_id}/stages", response_model=list[F005StageResponse])
async def list_stages(
    job_id: int,
    db: aiosqlite.Connection = Depends(get_db),
) -> list[F005StageResponse]:
    """job_id에 속한 모든 F005 스테이지를 stage_order ASC로 반환한다."""
    # job 존재 여부 확인
    job = await f005_service.get_job(db, job_id)
    if job is None:
        raise HTTPException(status_code=404, detail=f"Job {job_id} 를 찾을 수 없습니다.")
    stages = await f005_service.list_stages(db, job_id)
    return [F005StageResponse.model_validate(s) for s in stages]


@router.get("/jobs/{job_id}/stages/{stage_id}", response_model=F005StageResponse)
async def get_stage(
    job_id: int,
    stage_id: str,
    db: aiosqlite.Connection = Depends(get_db),
) -> F005StageResponse:
    """job_id + stage_id로 F005 스테이지 단건을 조회한다."""
    stage = await f005_service.get_stage(db, job_id, stage_id)
    if stage is None:
        raise HTTPException(
            status_code=404,
            detail=f"Job {job_id}의 Stage {stage_id} 를 찾을 수 없습니다.",
        )
    return F005StageResponse.model_validate(stage)
...
async def retry_stage(
    job_id: int,
    stage_id: str,
    request: F005StageRetryRequest,
    db: aiosqlite.Connection = Depends(get_db),
) -> dict:
    """F005 스테이지를 PENDING 상태로 리셋하고 override_params를 input_data에 반영한다."""
    stage = await f005_service.get_stage(db, job_id, stage_id)
    if stage is None:
        raise HTTPException(
            status_code=404,
...
async def reject_stage(
    job_id: int,
    stage_id: str,
    request: F005StageRejectRequest,
    db: aiosqlite.Connection = Depends(get_db),
) -> dict:
    """현재 F005 스테이지를 REJECTED로 표시하고 rejection_target 스테이지를 PENDING으로 리셋한다."""
    stage = await f005_service.get_stage(db, job_id, stage_id)
    if stage is None:
        raise HTTPException(
            status_code=404,
    ...
    await f005_service.update_stage_status(
        db,
        job_id,
        stage_id,
        status="REJECTED",
    ...
    if target_id:
        target_stage = await f005_service.get_stage(db, job_id, target_id)
        if target_stage:
            await db.execute(
                """
                UPDATE stages
...
async def approve_job(
    job_id: int,
    request: F005ApproveRequest,
    db: aiosqlite.Connection = Depends(get_db),
) -> dict:
    """PENDING_APPROVAL 상태인 F005 job을 YouTube에 직접 업로드하고 DONE 처리한다.
    ...
    stage_05 = await f005_service.get_stage(db, job_id, "STAGE_05_EDIT")
    ...
    stage_06 = await f005_service.get_stage(db, job_id, "STAGE_06_UPLOAD")
...
async def rerun_from_tts(
    job_id: int,
    request: F005ScriptRerunRequest,
    db: aiosqlite.Connection = Depends(get_db),
) -> dict:
    ...
    stage_02 = await f005_service.get_stage(db, job_id, "STAGE_02_SCRIPT")
