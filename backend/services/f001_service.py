# 목적: F001 content_jobs, stages 테이블 CRUD 및 오케스트레이터 실행 서비스
import sys
...
from schemas.f001 import F001JobCreateRequest

# F001 오케스트레이터 진입점 — 동적 경로 계산 (ERR-007 방지)
ORCHESTRATOR_PATH = str(
    Path(__file__).parent.parent.parent / "pipelines" / "f001_youtube" / "run_orchestrator.py"
)

# F001 파이프라인 6개 스테이지 정의 (stage_id, stage_order)
_STAGES = [
    ("STAGE_01_RESEARCH",  1),
    ("STAGE_02_SCRIPT",    2),
    ("STAGE_03_TTS",       3),
...
class F001Service:
    """F001 content_jobs, stages 테이블 CRUD 및 오케스트레이터 실행 서비스 클래스."""
    ...
    def _spawn_orchestrator(self, job_id: int) -> None:
        """subprocess.Popen으로 F001 오케스트레이터를 백그라운드 독립 프로세스로 실행한다.
        ...
        try:
            ...
            print(
                f"[F001Service] 오케스트레이터 실행 실패 job_id={job_id}: {exc}",
                file=sys.stderr,
            )
    ...
    async def list_jobs(
        ...
        """cursor 기반 페이징으로 content_jobs 목록을 최신 순(id DESC)으로 조회한다.
