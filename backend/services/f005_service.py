# 목적: F005 content_jobs, stages 테이블 CRUD 및 오케스트레이터 실행 서비스
import sys
...
from schemas.f005 import F005JobCreateRequest

# F005 오케스트레이터 진입점 — 동적 경로 계산 (ERR-007 방지)
ORCHESTRATOR_PATH = str(
    Path(__file__).parent.parent.parent / "pipelines" / "f005_youtube_v3" / "run_orchestrator.py"
)

# F005 파이프라인 6개 스테이지 정의 (stage_id, stage_order)
# STAGE_01_INPUT: 채팅 기반 입력 확정 (F004의 STAGE_01_RESEARCH 대신)
_STAGES = [
    ("STAGE_01_INPUT",    1),
    ("STAGE_02_SCRIPT",   2),
    ("STAGE_03_TTS",      3),
...
class F005Service:
    """F005 content_jobs, stages 테이블 CRUD 및 오케스트레이터 실행 서비스 클래스."""
    ...
    async def create_job(
        ...
        # content_jobs INSERT — feature_id = 'F005'
    ...
    def _spawn_orchestrator(self, job_id: int) -> None:
        """subprocess.Popen으로 F005 오케스트레이터를 백그라운드 독립 프로세스로 실행한다.
        ...
        try:
            ...
            print(
                f"[F005Service] 오케스트레이터 실행 실패 job_id={job_id}: {exc}",
                file=sys.stderr,
            )
    ...
    async def list_jobs(
        ...
        """cursor 기반 페이징으로 F005 content_jobs 목록을 최신 순(id DESC)으로 조회한다.
        ...
        if cursor is not None and status is not None:
            db_cursor = await db.execute(
                "SELECT * FROM content_jobs WHERE feature_id = 'F005' AND id < ? AND status = ? ORDER BY id DESC LIMIT ?",
                (cursor, status, fetch_limit),
            )
        elif cursor is not None:
            db_cursor = await db.execute(
                "SELECT * FROM content_jobs WHERE feature_id = 'F005' AND id < ? ORDER BY id DESC LIMIT ?",
                (cursor, fetch_limit),
            )
        elif status is not None:
            db_cursor = await db.execute(
                "SELECT * FROM content_jobs WHERE feature_id = 'F005' AND status = ? ORDER BY id DESC LIMIT ?",
                (status, fetch_limit),
            )
        else:
            db_cursor = await db.execute(
                "SELECT * FROM content_jobs WHERE feature_id = 'F005' ORDER BY id DESC LIMIT ?",
                (fetch_limit,),
            )
