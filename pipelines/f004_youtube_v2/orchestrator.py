# 목적: F004 6스테이지 오케스트레이터 — content_jobs + stages 테이블 기반 순차 실행.
...
# 스테이지 클래스 임포트
from pipelines.f004_youtube_v2.stages.stage01_research import Stage01Research
from pipelines.f004_youtube_v2.stages.stage02_script import Stage02Script
from pipelines.f004_youtube_v2.stages.stage03_tts import Stage03TTS
from pipelines.f004_youtube_v2.stages.stage04_video import Stage04VideoGen
from pipelines.f004_youtube_v2.stages.stage05_edit import Stage05Edit
from pipelines.f004_youtube_v2.stages.stage06_upload import Stage06Upload
...
# DB 경로 상수 — f004_youtube_v2/orchestrator.py 기준 세 단계 상위가 프로젝트 루트
...
class F004Orchestrator(BasePipeline):
    """F004 6스테이지 오케스트레이터.
    ...
    def get_metadata(self) -> dict:
        """F004 오케스트레이터 메타데이터."""
        return {
            "feature_id": "F004_MULTI",
            "name": "F004 멀티스테이지 오케스트레이터",
            "description": "6스테이지 유튜브 컨텐츠 제작 V2 파이프라인",
            "input_schema": {},
    ...
    def run(self, job_id: int, params: dict = None) -> dict:  # type: ignore[override]
        ...
        F004Orchestrator는 content_jobs 기반이므로 job_id만 받아 DB에서 로드한다.
        ...
        """
        logger.info(f"[F004Orchestrator] 시작 — job_id={job_id}")
        ...
        try:
            ...
            logger.info(f"[F004Orchestrator][job_id={job_id}] content_jobs RUNNING 전환")
            ...
            for stage_id, stage_order in STAGE_SEQUENCE:
                logger.info(
                    f"[F004Orchestrator][job_id={job_id}] "
            ...
            conn.commit()
            logger.info(
                f"[F004Orchestrator][job_id={job_id}] 모든 스테이지 완료 — "
            ...
            logger.error(f"[F004Orchestrator][job_id={job_id}] 오케스트레이터 실패: {e}")
            now = datetime.now(timezone.utc).isoformat()
            try:
                self._db_update(
                    conn,
                ...
                logger.error(f"[F004Orchestrator][job_id={job_id}] FAILED 업데이트 실패: {db_e}")
            raise
    ...
    def _load_config(self) -> dict:
        """f004_youtube_v2/config.json을 읽어 반환. 실패 시 빈 딕셔너리."""
    ...
    def _get_stage_input(
        ...
        if stage_id == "STAGE_04_VIDEO_GEN":
            ...
            return {
                **initial_params,
                # F004는 slides 기반 PPT 렌더링 — scenes는 폴백용으로 함께 전달
                ...
                # skip_mode는 F004에서 사용 안하지만 호환성 유지
