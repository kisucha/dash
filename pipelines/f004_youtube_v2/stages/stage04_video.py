# 목적: F004 STAGE_04 — Pillow 기반 PPT 슬라이드 PNG 이미지 생성.
...
def _load_font(
    ...
    Args:
        size: 폰트 포인트 크기
        bold: True이면 Bold 후보를 앞에 배치
        extra_candidates: 전역 FONT_CANDIDATES 앞에 추가할 경로 목록 (전역 변수 오염 방지)
    """
...
class SlideRenderer:
    ...
    def _make_base_image(self, gradient: bool = False):
        ...
        gradient=True이면 numpy로 대각선 그라디언트를 그린다.
        numpy 미설치 시 단색 배경으로 폴백한다.
        ...
        if gradient:
            try:
                ...
                # 배경색과 accent색 사이 대각선 그라디언트 (18% 혼합)
    ...
    def _draw_footer(self, draw, source: str, page: int, total: int) -> None:
        ...
        Args:
            draw: ImageDraw 인스턴스
            source: 출처 문자열 (최대 60자 표시)
            page: 현재 페이지 번호 (미사용 — 확장 여지)
            total: 전체 슬라이드 수 (미사용 — 확장 여지)
        """
        fy = H - FOOTER_H
    ...
    def _wrap_text(self, draw, text: str, font, max_width: int) -> list:
        ...
        글자 단위로 붙여가며 폭을 측정해 초과하면 새 줄을 시작한다.

        Args:
            draw: ImageDraw 인스턴스 (textbbox 사용)
            text: 원본 텍스트
            font: 측정에 사용할 폰트
            max_width: 최대 픽셀 폭
    ...
    def render_title(self, slide: dict, page: int, total: int) -> "Image.Image":
        ...
        그라디언트 배경 + 중앙 대제목 + accent 언더라인 + 부제목.

        Args:
            slide: {title, subtitle} 딕셔너리
            page: 현재 페이지 번호
            total: 전체 슬라이드 수

        Returns:
            1280x720 PIL Image 객체
        """
        ...
        # 헤더 (타이틀 슬라이드는 채널명 고정)
        self._draw_header(draw, "유튜브 컨텐츠 제작 V2", page, total)
    ...
    def render_content(self, slide: dict, page: int, total: int) -> "Image.Image":
        ...
        헤더 + 불릿 포인트 목록(최대 5개) + 푸터.

        Args:
            slide: {title, bullets, source} 딕셔너리
            page: 현재 페이지 번호
            total: 전체 슬라이드 수
    ...
    def render_summary(self, slide: dict, page: int, total: int) -> "Image.Image":
        ...
        bullets = slide.get("bullets", [])
        nfont = self._font(50, bold=True)   # 번호 폰트
        tfont = self._font(32)              # 내용 폰트
    ...
    def render_quote(self, slide: dict, page: int, total: int) -> "Image.Image":
        ...
        중앙 정렬된 인용문 또는 핵심 수치를 accent_light 색상으로 강조한다.

        Args:
            slide: {title, quote, bullets, source} 딕셔너리
            page: 현재 페이지 번호
            total: 전체 슬라이드 수
        ...
        # quote 필드 우선, 없으면 bullets 첫 항목 사용
        quote = slide.get(
            "quote",
...
class Stage04VideoGen(BaseStage, BasePipeline):
    """F004 STAGE_04 — Pillow 기반 PPT 슬라이드 이미지 생성.
    ...
    def get_metadata(self) -> dict:
        """BasePipeline 추상 메서드 충족용 — 파이프라인 식별 메타데이터."""
        return {"feature_id": "F004_STAGE04", "name": "STAGE_04_VIDEO_GEN"}

    def run(self, task_id: int, params: dict) -> dict:
        """BasePipeline 추상 메서드 충족용 — execute에 위임."""
    ...
    def validate_input(self, data: dict) -> ValidationResult:
        ...
        Returns:
            ValidationResult — slides/scenes가 없으면 STAGE_02 반송
        """
        # 크리틱 지적 반영: slides OR scenes 양쪽 키 지원
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        Args:
            job_id: content_jobs.id — 저장 경로 구성에 사용
            input_data: {
        ...
        """
        logger.info(f"[F004][STAGE_04][job_id={job_id}] 슬라이드 생성 시작")

        # 크리틱 지적 반영: slides 우선, 없으면 scenes 폴백
        ...
        # config.json slide_font_path — 전역 변수 오염 없이 SlideRenderer에 주입
        custom_font = input_data.get("slide_font_path", "")
        if custom_font:
            logger.info(f"[F004][STAGE_04][job_id={job_id}] 사용자 지정 폰트 경로: {custom_font}")

        # 출력 디렉토리 — 절대 경로 (ERR-007 방지)
        project_root = Path(__file__).parent.parent.parent.parent
        output_dir = (
            project_root / "storage" / "results" / "f004" / str(job_id) / "slides"
        )
        output_dir.mkdir(parents=True, exist_ok=True)

        # 테마 설정 — 미등록 테마명이면 기본값으로 폴백
        ...
        for slide in slides:
            ...
            try:
                ...
                # 크리틱 지적 반영: narration 필드 포함 — STAGE_05 비례 배분용
                ...
                logger.info(
                    f"[F004][STAGE_04][job_id={job_id}] 슬라이드 {slide_no} 완료"
                )

            except Exception as e:
                logger.warning(
                    f"[F004][STAGE_04][job_id={job_id}] 슬라이드 {slide_no} 실패: {e}"
                )
        ...
        if not clips:
            ...
            try:
                ...
                logger.warning(
                    f"[F004][STAGE_04][job_id={job_id}] 폴백 슬라이드 생성 완료"
                )
            except Exception as fe:
                logger.error(
                    f"[F004][STAGE_04] 폴백 슬라이드 생성 실패: {fe}"
                )
        ...
        if clips:
            ...
            try:
                ...
                logger.warning(
                    f"[F004][STAGE_04] 썸네일 복사 실패: {te}"
                )
                thumbnail_path = None

        logger.info(
            f"[F004][STAGE_04][job_id={job_id}] 완료 — 슬라이드 {len(clips)}장"
        )
    ...
    def validate_output(self, output: dict) -> ValidationResult:
        ...
        Returns:
            ValidationResult — clips 없으면 is_valid=False
        """
