...
class Stage01Research(BaseStage, BasePipeline):
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F004_STAGE01", "name": "STAGE_01_RESEARCH"}
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        logger.info(
            f"[F004][STAGE_01][job_id={job_id}] 실행 시작 — "
            f"카테고리: {channel_category!r}, 후보 수: {target_count}"
        )

        # days → SearXNG time_range 변환
        if days <= 2:
            time_range = "day"
        elif days <= 7:
            time_range = "week"
        ...
        # SearXNG 검색 결과 수집 — time_range + max_age_days 날짜 필터 적용
        ...
        if "searxng" in search_provider:
            ...
            try:
                ...
                logger.info(
                    f"[F004][STAGE_01][job_id={job_id}] SearXNG 검색 완료 — "
                    f"{len(searxng_results)}건 수신 (time_range={time_range}, max_age={days}일)"
                )
            except Exception as e:
                # SearXNG 실패는 경고만 기록하고 Ollama 단독으로 계속 진행
                logger.warning(
                    f"[F004][STAGE_01][job_id={job_id}] SearXNG 검색 실패 (Ollama 단독 진행): {e}"
                )

        # 검색 결과를 프롬프트 컨텍스트로 변환 (최대 3000자, 날짜 포함)
        search_context: str = self._build_search_context(searxng_results)

        # 오늘 날짜 (UTC 기준 KST+9 보정 없이 UTC 사용 — 충분히 근사)
        today_str: str = datetime.now(timezone.utc).strftime("%Y-%m-%d")

        # Ollama 프롬프트 구성 — 날짜 제약 명시
        prompt: str = (
            f"당신은 유튜브 컨텐츠 전략 전문가입니다.\n"
            f"오늘 날짜: {today_str}\n"
            f"채널 카테고리: {channel_category}\n"
            f"아래 트렌드 데이터는 오늘 기준 최근 {days}일 이내에 수집된 최신 정보입니다:\n"
            f"{search_context}\n\n"
            f"[중요 제약사항]\n"
            f"- 반드시 {today_str} 기준 최근 {days}일 이내의 트렌드를 반영하세요.\n"
            f"- {days}일보다 오래된 이슈나 사건은 주제로 선정하지 마세요.\n"
            f"- 특정 날짜나 연도를 언급할 때는 {today_str[:4]}년 기준으로 작성하세요.\n\n"
        ...
        logger.info(f"[F004][STAGE_01][job_id={job_id}] Ollama 주제 생성 요청 시작")
        ...
        try:
            ...
            logger.error(f"[F004][STAGE_01][job_id={job_id}] Ollama 호출 실패: {e}")
        ...
        logger.info(
            f"[F004][STAGE_01][job_id={job_id}] 주제 후보 생성 완료 — "
            f"{len(topics)}개"
        )
