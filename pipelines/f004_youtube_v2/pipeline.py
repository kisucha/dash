# F004 유튜브 컨텐츠 제작 V2 파이프라인 — 주제와 스타일을 입력받아 Ollama로 영상 스크립트를 생성한다.
...
class F004Pipeline(BasePipeline):
    """
    F004 유튜브 컨텐츠 제작 V2 파이프라인.
    ...
    def run(self, task_id: int, params: dict) -> dict:
        ...
        """
        logger.info(f"[F004][task_id={task_id}] 파이프라인 실행 시작")
        ...
        if style not in allowed_styles:
            logger.warning(f"[F004] 알 수 없는 style '{style}' — 'educational'로 대체")
            style = "educational"
        ...
        try:
            ...
            logger.warning("[F004] duration_min 파싱 실패 — 기본값 10분 사용")
            duration_min = 10
        ...
        if self.is_cancelled(task_id):
            logger.info(f"[F004][task_id={task_id}] 실행 전 취소 감지 — 중단")
            return {}
        ...
        # ------------------------------------------------------------------
        logger.info(f"[F004][task_id={task_id}] 1단계: 제목 생성 중...")
        self.update_status(
            task_id=task_id,
            status="RUNNING",
        ...
        if self.is_cancelled(task_id):
            logger.info(f"[F004][task_id={task_id}] 제목 생성 전 취소 감지")
            return {}
        ...
        logger.info(f"[F004][task_id={task_id}] 제목 생성 완료: {title!r}")

        # ------------------------------------------------------------------
        # [5-B] 영상 설명문 생성 — Ollama 2차 호출
        # ------------------------------------------------------------------
        logger.info(f"[F004][task_id={task_id}] 2단계: 영상 설명문 생성 중...")
        self.update_status(
            task_id=task_id,
            status="RUNNING",
        ...
        if self.is_cancelled(task_id):
            logger.info(f"[F004][task_id={task_id}] 설명문 생성 전 취소 감지")
            return {"title": title}
        ...
        logger.info(f"[F004][task_id={task_id}] 설명문 생성 완료 ({len(description)}자)")

        # ------------------------------------------------------------------
        # [5-C] 스크립트 생성 — Ollama 3차 호출 (가장 오래 걸림)
        # ------------------------------------------------------------------
        logger.info(f"[F004][task_id={task_id}] 3단계: 영상 스크립트 생성 중...")
        self.update_status(
            task_id=task_id,
            status="RUNNING",
            result={
        ...
        if self.is_cancelled(task_id):
            logger.info(f"[F004][task_id={task_id}] 스크립트 생성 전 취소 감지")
        ...
        logger.info(f"[F004][task_id={task_id}] 스크립트 생성 완료 ({len(script)}자)")
        ...
        logger.info(f"[F004][task_id={task_id}] 파이프라인 완료")
        return result
