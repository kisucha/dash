# 목적: 레거시 tasks 테이블 F004 이력을 content_jobs + stages로 선택적 마이그레이션하는 유틸.
# 원본 tasks 레코드는 삭제하지 않고 legacy_task_id로 연결만 한다.
# backend/routers/f004.py의 POST /api/f004/migrate-legacy 엔드포인트에서 호출된다.
...
def migrate_f004_tasks(task_ids: Optional[list[int]] = None) -> dict:
    """레거시 tasks 테이블의 F004 이력을 content_jobs + stages로 마이그레이션.
    ...
    Args:
        task_ids: 마이그레이션할 task_id 목록. None이면 F004 전체.
    ...
    try:
        ...
        if task_ids:
            ...
            cursor = conn.execute(
                f"SELECT * FROM tasks WHERE feature_id = 'F004' AND id IN ({placeholders})",
                task_ids,
            )
        else:
            cursor = conn.execute(
                "SELECT * FROM tasks WHERE feature_id = 'F004'"
            )

        tasks = [dict(row) for row in cursor.fetchall()]
        logger.info(f"[migrate_f004] 마이그레이션 대상 tasks: {len(tasks)}건")
        ...
        for task in tasks:
            ...
            if dup_cursor.fetchone():
                logger.info(f"[migrate_f004] task_id={task_id} 이미 마이그레이션됨 — 건너뜀")
                skipped_count += 1
                details.append({
                    "task_id": task_id,
                    "result": "skipped",
            ...
            if result_raw:
                try:
                    ...
                    logger.info(
                        f"[migrate_f004] task_id={task_id} STAGE_02 output_data 저장 완료"
                    )
                except (json.JSONDecodeError, TypeError, KeyError) as e:
                    logger.warning(
                        f"[migrate_f004] task_id={task_id} result 파싱 실패 — "
            ...
            logger.info(
                f"[migrate_f004] task_id={task_id} → job_id={job_id} 마이그레이션 완료"
            )

    except Exception as e:
        conn.rollback()
        logger.error(f"[migrate_f004] 마이그레이션 실패: {e}")
        raise
    ...
    logger.info(
        f"[migrate_f004] 완료 — 성공: {migrated_count}건, 건너뜀: {skipped_count}건"
    )
    return {
...
if __name__ == "__main__":
    """직접 실행 시 F004 전체 마이그레이션 수행."""
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
        encoding="utf-8",
    )
    result = migrate_f004_tasks()
