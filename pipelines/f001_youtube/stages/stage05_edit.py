...
class Stage05Edit(BaseStage, BasePipeline):
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F001_STAGE05", "name": "STAGE_05_EDIT"}
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        Args:
            ...
            input_data: {
                ...
                clips (list): [{scene_no, file_path, duration_sec}]
        ...
        if input_data.get("stage05_auto_skipped", False):
            logger.info(
                f"[STAGE_05][job_id={job_id}] STAGE_04 script_only로 인해 자동 SKIPPED"
            )
            return {
        ...
        if not clips and not audio_file_path:
            logger.info(
                f"[STAGE_05][job_id={job_id}] 클립과 오디오 모두 없어 SKIPPED"
            )
            return {
                "stage_id": "STAGE_05_EDIT",
                "status": "SKIPPED",
        ...
        output_dir = _project_root / "storage" / "results" / "f001" / str(job_id) / "final"
        output_dir.mkdir(parents=True, exist_ok=True)
        output_video: str = str(output_dir / "output.mp4")
        output_srt: str = str(output_dir / "subtitles.srt")

        logger.info(
            f"[STAGE_05][job_id={job_id}] FFmpeg 편집 시작 — "
        ...
        try:
            ...
            logger.error(f"[STAGE_05][job_id={job_id}] FFmpeg 실패: {e}")
        ...
        logger.info(
            f"[STAGE_05][job_id={job_id}] 편집 완료 — "
    ...
    def _get_audio_duration_sec(self, audio_path: str) -> float:
        ...
        try:
            ...
            logger.warning(f"[STAGE_05] moviepy 오디오 길이 측정 실패: {e}")
        ...
        try:
            ...
            logger.warning(f"[STAGE_05] ffprobe 오디오 길이 측정 실패: {e}")
        return 0.0
    ...
    def _run_ffmpeg_concat(
        ...
        if has_audio and audio_duration > 0 and valid_clips:
            ...
            logger.info(
                f"[STAGE_05][job_id={job_id}] 오디오 기준 클립 재배분 — "
        ...
        for clip in clips:
            ...
            if not Path(file_path).exists():
                logger.warning(
                    f"[STAGE_05][job_id={job_id}] 클립 파일 없음 건너뜀: {file_path}"
                )
                continue
        ...
        logger.info(
            f"[STAGE_05][job_id={job_id}] FFmpeg 실행 — "
        ...
        logger.info(f"[STAGE_05][job_id={job_id}] FFmpeg concat 완료: {output_video}")
    ...
    def _generate_black_video_with_audio(
        ...
        logger.info(f"[STAGE_05][job_id={job_id}] 검정 배경 영상 생성 완료")
    ...
    def _run_whisper_transcribe(
        ...
        try:
            ...
            logger.warning(
                f"[STAGE_05][job_id={job_id}] openai-whisper 미설치 — 자막 생성 건너뜀. "
        ...
        try:
            logger.info(
                f"[STAGE_05][job_id={job_id}] Whisper 자막 생성 시작: {audio_file_path}"
            )
            ...
            if not segments:
                logger.warning(
                    f"[STAGE_05][job_id={job_id}] Whisper 세그먼트 없음 — 자막 없음"
                )
                return False

            self._write_srt(segments, output_srt)
            logger.info(
                f"[STAGE_05][job_id={job_id}] Whisper 자막 생성 완료: {output_srt}"
            )
            return True

        except Exception as e:
            logger.warning(
                f"[STAGE_05][job_id={job_id}] Whisper 실패 — 자막 없이 계속: {e}"
            )
            return False
    ...
    def _get_video_duration(self, video_path: str) -> float:
        ...
        try:
            ...
            duration = round(float(clip.duration), 1)
            clip.close()
            return duration
        except Exception as e:
