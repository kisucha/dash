...
class Stage05Edit(BaseStage, BasePipeline):
    ...
    처리 흐름:
      ...
      5. 최종 output.mp4 및 subtitles.srt 저장 (f005 경로 사용)
    """
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F005_STAGE05", "name": "STAGE_05_EDIT"}
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        if input_data.get("stage05_auto_skipped", False):
            logger.info(
                f"[F005][STAGE_05][job_id={job_id}] STAGE_04 script_only로 인해 자동 SKIPPED"
            )
            return {
        ...
        if not clips and not audio_file_path:
            logger.info(
                f"[F005][STAGE_05][job_id={job_id}] 클립과 오디오 모두 없어 SKIPPED"
            )
            return {
                "stage_id": "STAGE_05_EDIT",
                "status": "SKIPPED",
        ...
        output_dir = _project_root / "storage" / "results" / "f005" / str(job_id) / "final"
        output_dir.mkdir(parents=True, exist_ok=True)
        output_video: str = str(output_dir / "output.mp4")
        output_srt: str = str(output_dir / "subtitles.srt")

        logger.info(
            f"[F005][STAGE_05][job_id={job_id}] FFmpeg 편집 시작 — "
        ...
        try:
            ...
            logger.error(f"[F005][STAGE_05][job_id={job_id}] FFmpeg 실패: {e}")
        ...
        logger.info(
            f"[F005][STAGE_05][job_id={job_id}] 편집 완료 — "
    ...
    def _distribute_duration_by_narration(
        ...
        if floor_total >= audio_duration:
            ...
            logger.warning(
                f"[F005][STAGE_05] min {min_dur}s 보장으로 audio_duration({audio_duration:.1f}s) "
        ...
        if floor_indices:
            logger.info(
                f"[F005][STAGE_05] min {min_dur}s 클램핑 발동: {len(floor_indices)}개 클립, "
    ...
    def _get_audio_duration_sec(self, audio_path: str) -> float:
        ...
        try:
            ...
            logger.warning(f"[F005][STAGE_05] moviepy 오디오 길이 측정 실패: {e}")
        try:
            ...
            logger.warning(f"[F005][STAGE_05] ffprobe 오디오 길이 측정 실패: {e}")
        return 0.0
    ...
    def _run_ffmpeg_concat(
        ...
        if has_audio and audio_duration > 0 and valid_clips:
            ...
            if has_narration:
                ...
                logger.info(
                    f"[F005][STAGE_05][job_id={job_id}] narration 비례 배분 적용 — "
                ...
                logger.info(
                    f"[F005][STAGE_05][job_id={job_id}] 오디오 기준 균등 배분 — "
                    f"오디오 {audio_duration:.1f}초 / {len(valid_clips)}클립 "
        ...
        for clip in clips:
            ...
            if not Path(file_path).exists():
                logger.warning(
                    f"[F005][STAGE_05][job_id={job_id}] 클립 파일 없음 건너뜀: {file_path}"
                )
                continue
        ...
        logger.info(
            f"[F005][STAGE_05][job_id={job_id}] FFmpeg 실행 — "
        ...
        logger.info(f"[F005][STAGE_05][job_id={job_id}] FFmpeg concat 완료: {output_video}")
    ...
    def _generate_black_video_with_audio(
        ...
        logger.info(f"[F005][STAGE_05][job_id={job_id}] 검정 배경 영상 생성 완료")
    ...
    def _run_whisper_transcribe(
        ...
        try:
            ...
            logger.warning(
                f"[F005][STAGE_05][job_id={job_id}] openai-whisper 미설치 — 자막 생성 건너뜀. "
        ...
        try:
            logger.info(
                f"[F005][STAGE_05][job_id={job_id}] Whisper 자막 생성 시작: {audio_file_path}"
            )
            ...
            if not segments:
                logger.warning(
                    f"[F005][STAGE_05][job_id={job_id}] Whisper 세그먼트 없음 — 자막 없음"
                )
                return False

            self._write_srt(segments, output_srt)
            logger.info(
                f"[F005][STAGE_05][job_id={job_id}] Whisper 자막 생성 완료: {output_srt}"
            )
            return True

        except Exception as e:
            logger.warning(
                f"[F005][STAGE_05][job_id={job_id}] Whisper 실패 — 자막 없이 계속: {e}"
            )
            return False
