# 목적: F005 STAGE_03 — TTS 보이스오버 생성 스테이지.
...
class Stage03TTS(BaseStage, BasePipeline):
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F005_STAGE03", "name": "STAGE_03_TTS"}
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        if tts_skip:
            logger.info(f"[F005][STAGE_03][job_id={job_id}] TTS 건너뜀 (사용자 선택)")
            return {
        ...
        if script_text.strip().startswith("{"):
            try:
                ...
                if _parts:
                    ...
                    logger.info(f"[F005][STAGE_03][job_id={job_id}] script_text JSON → narration 추출 ({len(script_text)}자)")
            except Exception:
                pass
        ...
        if not script_text.strip():
            logger.warning(
                f"[F005][STAGE_03][job_id={job_id}] 스크립트 텍스트가 비어 있어 TTS 건너뜀"
            )
            return {
                "stage_id": "STAGE_03_TTS",
                "status": "SKIPPED",
        ...
        output_path: str = str(_project_root / "storage" / "results" / "f005" / str(job_id) / f"voiceover.{_audio_ext}")
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)

        logger.info(
            f"[F005][STAGE_03][job_id={job_id}] TTS 시작 — "
        ...
        try:
            ...
            logger.error(f"[F005][STAGE_03][job_id={job_id}] TTS 실패: {e}")
        ...
        logger.info(
            f"[F005][STAGE_03][job_id={job_id}] TTS 완료 — "
        ...
        try:
            ...
            logger.warning(f"[F005][STAGE_03][job_id={job_id}] SRT 생성 실패 (무시): {e}")
    ...
    def _run_voicebox_tts(
        ...
        logger.info(
            f"[F005][STAGE_03][job_id={job_id}] VoiceBox TTS 시작 - "
        ...
        logger.info(f"[F005][STAGE_03][job_id={job_id}] VoiceBox TTS 완료")
    ...
    def _run_coqui_tts(self, script_text: str, output_path: str, job_id: int) -> None:
        ...
        """
        logger.info(f"[F005][STAGE_03][job_id={job_id}] Coqui TTS subprocess 실행 시작")
        ...
        logger.info(f"[F005][STAGE_03][job_id={job_id}] Coqui TTS 완료")
    ...
    def _run_edge_tts(
        ...
        logger.info(
            f"[F005][STAGE_03][job_id={job_id}] Edge TTS 실행 시작 — "
        ...
        try:
            ...
            logger.info(f"[F005][STAGE_03][job_id={job_id}] Edge TTS 완료")
        except Exception as e:
    ...
    def _run_gtts(self, script_text: str, output_path: str, job_id: int) -> None:
        ...
        logger.info(f"[F005][STAGE_03][job_id={job_id}] Google TTS 실행 시작")
        try:
        ...
        try:
            ...
            logger.info(f"[F005][STAGE_03][job_id={job_id}] Google TTS 완료")
        except Exception as e:
    ...
    def _run_elevenlabs_tts(
        ...
        logger.info(f"[F005][STAGE_03][job_id={job_id}] ElevenLabs TTS API 호출 시작 — voice={voice_id}")
        try:
            ...
            logger.info(f"[F005][STAGE_03][job_id={job_id}] ElevenLabs TTS 완료")
    ...
    def _run_openai_tts(
        ...
        logger.info(f"[F005][STAGE_03][job_id={job_id}] OpenAI TTS API 호출 시작 — voice={_voice}")
        try:
            with httpx.Client(timeout=120.0) as client:
                resp = client.post(
            ...
            logger.info(f"[F005][STAGE_03][job_id={job_id}] OpenAI TTS 완료")
    ...
    def _generate_srt(
        ...
        logger.info(
            f"[F005][STAGE_03][job_id={job_id}] SRT 생성 완료 — "
