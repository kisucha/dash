# 목적: F005 STAGE_02 — 슬라이드 친화적 유튜브 스크립트 생성
...
class Stage02Script(BaseStage, BasePipeline):
    ...
    처리 흐름:
      1. STAGE_01에서 자동 확정된 selected_topic 확인
      2. 슬라이드 수 / 목표 글자 수 계산
      3. Ollama로 PPT 슬라이드 구조 JSON 스크립트 생성 (단일 호출)
      4. slides 배열 파싱 + script_text 조합 + 분량 검증

    F005에서는 selected_topic이 STAGE_01에서 자동 확정되므로
    validate_input이 STAGE_01_INPUT으로 반송 요청을 반환한다.
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F005_STAGE02", "name": "STAGE_02_SCRIPT"}
    ...
    def validate_input(self, data: dict) -> ValidationResult:
        ...
        selected_topic이 없으면 STAGE_01_INPUT으로 반송.
        F005에서는 STAGE_01이 항상 selected_topic을 자동 확정하므로
        이 검증이 실패하는 경우는 STAGE_01 오류를 의미한다.
        """
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        Args:
            ...
            input_data: {
                selected_topic (str, 필수): STAGE_01에서 자동 확정된 주제 제목
                ...
                enriched_context (str, 기본 ""): Ollama enriched 컨텍스트 (STAGE_01 인계)
                topics (list, 기본 []): 주제 후보 목록 (세부 정보 추출용, F004 호환)
            }
        ...
        # F005: enriched_context 우선, 없으면 search_context 사용
        ...
        logger.info(
            f"[F005][STAGE_02][job_id={job_id}] 실행 시작 — "
        ...
        # 1단계: 컨텍스트 섹션 구성
        # ----------------------------------------------------------------
        # F005: enriched_context를 search_context보다 우선 사용
        ...
        logger.info(
            f"[F005][STAGE_02][job_id={job_id}] 슬라이드 스크립트 Ollama 호출 시작 — "
        ...
        try:
            ...
            logger.error(
                f"[F005][STAGE_02][job_id={job_id}] 슬라이드 생성 Ollama 실패: {e}"
            )
        ...
        logger.info(
            f"[F005][STAGE_02][job_id={job_id}] 슬라이드 파싱 완료 — {len(slides)}장"
        )
        ...
        logger.info(
            f"[F005][STAGE_02][job_id={job_id}] 완료 — "
    ...
    def _parse_slides_json(
            ...
            try:
                ...
                if isinstance(data, dict) and "slides" in data:
                    ...
                    if isinstance(slides, list) and len(slides) > 0:
                        logger.info(
                            f"[F005][STAGE_02] JSON 파싱 성공 (dict.slides) — {len(slides)}장"
                        )
                        return slides
        ...
        if bracket_start != -1 and bracket_end != -1:
            ...
            try:
                ...
                if isinstance(data_list, list) and len(data_list) > 0:
                    logger.info(
                        f"[F005][STAGE_02] JSON 파싱 성공 (list) — {len(data_list)}장"
                    )
                    return data_list
            except (json.JSONDecodeError, ValueError):
                pass
        ...
        if match:
            try:
                ...
                if isinstance(slides_candidate, list) and len(slides_candidate) > 0:
                    logger.info(
                        f"[F005][STAGE_02] JSON 파싱 성공 (slides 부분 추출) — "
        ...
        logger.warning(
            f"[F005][STAGE_02] JSON 파싱 실패 — 기본 슬라이드 3장 생성 "
