# 목적: F005 STAGE_06 — SEO 최적화 메타데이터 생성 및 YouTube 업로드 스테이지.
...
class Stage06Upload(BaseStage, BasePipeline):
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F005_STAGE06", "name": "STAGE_06_UPLOAD"}
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        logger.info(
            f"[F005][STAGE_06][job_id={job_id}] 실행 시작 — "
        ...
        logger.info(f"[F005][STAGE_06][job_id={job_id}] SEO 메타데이터 생성 Ollama 호출 시작")
        try:
        ...
        if upload_mode == "auto" and video_file_path and Path(video_file_path).exists():
            logger.info(f"[F005][STAGE_06][job_id={job_id}] auto 모드 — YouTube 업로드 시도")
            ...
            try:
                ...
                logger.info(
                    f"[F005][STAGE_06][job_id={job_id}] YouTube 업로드 완료 — "
                    f"video_id: {youtube_video_id}"
                )
            except Exception as e:
                logger.error(f"[F005][STAGE_06][job_id={job_id}] YouTube 업로드 실패: {e}")
                raise RuntimeError(f"YouTube 업로드 실패: {e}") from e
        else:
            logger.info(
                f"[F005][STAGE_06][job_id={job_id}] manual_approval 모드 — "
                f"업로드 승인 대기 상태로 전환"
            )
