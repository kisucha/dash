...
# 스테이지 클래스 임포트
from pipelines.f005_youtube_v3.stages.stage01_input import Stage01Input
from pipelines.f005_youtube_v3.stages.stage02_script import Stage02Script
from pipelines.f005_youtube_v3.stages.stage03_tts import Stage03TTS
from pipelines.f005_youtube_v3.stages.stage04_video import Stage04VideoGen
from pipelines.f005_youtube_v3.stages.stage05_edit import Stage05Edit
from pipelines.f005_youtube_v3.stages.stage06_upload import Stage06Upload
...
# DB 경로 상수 — f005_youtube_v3/orchestrator.py 기준 세 단계 상위가 프로젝트 루트
...
class F005Orchestrator(BasePipeline):
    """F005 6스테이지 오케스트레이터.
    ...
    F004와의 핵심 차이:
      - STAGE_01_RESEARCH 대신 STAGE_01_INPUT 사용
      - STAGE_01 완료 후 WAITING 분기 없음 — selected_topic이 자동 확정됨
      - 사용자가 initial_params에 주제를 직접 입력하므로 대기 불필요
    ...
    def get_metadata(self) -> dict:
        """F005 오케스트레이터 메타데이터."""
        return {
            "feature_id": "F005_MULTI",
            "name": "F005 멀티스테이지 오케스트레이터",
            "description": "6스테이지 유튜브 컨텐츠 제작 V3 파이프라인 (주제 직접 입력 방식)",
            "input_schema": {},
    ...
    def run(self, job_id: int, params: dict = None) -> dict:  # type: ignore[override]
        ...
        F005Orchestrator는 content_jobs 기반이므로 job_id만 받아 DB에서 로드한다.
        ...
        """
        logger.info(f"[F005Orchestrator] 시작 — job_id={job_id}")
        ...
        try:
            ...
            logger.info(f"[F005Orchestrator][job_id={job_id}] content_jobs RUNNING 전환")
            ...
            for stage_id, stage_order in STAGE_SEQUENCE:
                logger.info(
                    f"[F005Orchestrator][job_id={job_id}] "
                ...
                # F005는 STAGE_01_INPUT이 selected_topic을 자동 확정하므로
                # F004의 WAITING 체크 로직 없음 — 모든 스테이지 연속 진행
            ...
            conn.commit()
            logger.info(
                f"[F005Orchestrator][job_id={job_id}] 모든 스테이지 완료 — "
            ...
            logger.error(f"[F005Orchestrator][job_id={job_id}] 오케스트레이터 실패: {e}")
            now = datetime.now(timezone.utc).isoformat()
            try:
                self._db_update(
                    conn,
                ...
                logger.error(f"[F005Orchestrator][job_id={job_id}] FAILED 업데이트 실패: {db_e}")
            raise
    ...
    def _load_config(self) -> dict:
        """f005_youtube_v3/config.json을 읽어 반환. 실패 시 빈 딕셔너리."""
    ...
    def _get_stage_input(
        ...
        if stage_id == "STAGE_04_VIDEO_GEN":
            ...
            return {
                **initial_params,
                # F005는 slides 기반 PPT 렌더링 — scenes는 폴백용으로 함께 전달
