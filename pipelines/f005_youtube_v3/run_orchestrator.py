# 목적: F005 오케스트레이터 subprocess 진입점 — argv[1]=job_id.
# f005_service.create_job()에서 subprocess.Popen([sys.executable, this_file, str(job_id)])으로 호출된다.
...
from pipelines.f005_youtube_v3.orchestrator import F005Orchestrator
...
def main() -> None:
    ...
    logger.info(f"[run_orchestrator] f005 오케스트레이터 시작 — job_id={job_id}")

    try:
        orchestrator = F005Orchestrator()
        result = orchestrator.run(job_id)
        logger.info(f"[run_orchestrator] f005 오케스트레이터 완료 — job_id={job_id}, 결과: {result}")
        sys.exit(0)

    except Exception as e:
        logger.error(f"[run_orchestrator] f005 오케스트레이터 실패 — job_id={job_id}: {e}")
