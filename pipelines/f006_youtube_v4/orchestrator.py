...
# 스테이지 클래스 임포트
from pipelines.f006_youtube_v4.stages.stage01_input import Stage01Input
from pipelines.f006_youtube_v4.stages.stage02_script import Stage02Script
from pipelines.f006_youtube_v4.stages.stage03_tts import Stage03TTS
from pipelines.f006_youtube_v4.stages.stage04_video import Stage04VideoGen
from pipelines.f006_youtube_v4.stages.stage05_edit import Stage05Edit
from pipelines.f006_youtube_v4.stages.stage06_upload import Stage06Upload
...
# DB 경로 상수 - f006_youtube_v4/orchestrator.py 기준 세 단계 상위가 프로젝트 루트
...
class F006Orchestrator(BasePipeline):
    """F006 6스테이지 오케스트레이터.
    ...
    F005와의 독립 관계:
      - pipelines.f006_youtube_v4 경로만 임포트 (f005 코드 참조 금지)
      - 출력 경로: storage/results/f006/
      - STAGE_01_INPUT이 selected_topic을 자동 확정 (WAITING 없음)
    ...
    def get_metadata(self) -> dict:
        """F006 오케스트레이터 메타데이터."""
        return {
            "feature_id": "F006_MULTI",
            "name": "F006 멀티스테이지 오케스트레이터",
            "description": "6스테이지 유튜브 컨텐츠 제작 v4 파이프라인 (주제 직접 입력 방식)",
            "input_schema": {},
    ...
    def run(self, job_id: int, params: dict = None) -> dict:  # type: ignore[override]
        ...
        F006Orchestrator는 content_jobs 기반이므로 job_id만 받아 DB에서 로드한다.
        ...
        """
        logger.info(f"[F006Orchestrator] 시작 - job_id={job_id}")
        ...
        try:
            ...
            logger.info(f"[F006Orchestrator][job_id={job_id}] content_jobs RUNNING 전환")
            ...
            for stage_id, stage_order in STAGE_SEQUENCE:
                logger.info(
                    f"[F006Orchestrator][job_id={job_id}] "
                ...
                # F006는 STAGE_01_INPUT이 selected_topic을 자동 확정하므로
                # WAITING 체크 로직 없음 - 모든 스테이지 연속 진행
            ...
            conn.commit()
            logger.info(
                f"[F006Orchestrator][job_id={job_id}] 모든 스테이지 완료 - "
            ...
            logger.error(f"[F006Orchestrator][job_id={job_id}] 오케스트레이터 실패: {e}")
            now = datetime.now(timezone.utc).isoformat()
            try:
                self._db_update(
                    conn,
                ...
                logger.error(f"[F006Orchestrator][job_id={job_id}] FAILED 업데이트 실패: {db_e}")
            raise
    ...
    def _run_stage_by_id(
        ...
        Args:
            db_conn: sqlite3 커넥션
            job_id: content_jobs.id
            stage_id: 스테이지 ID 문자열 (예: "STAGE_01_INPUT")
        ...
        # 스테이지 status 결정
        # STAGE_06이 manual_approval 대기 중이면 PENDING_APPROVAL로 저장 -> 프론트 배너 표시
        final_stage_status = output_data.get("status", "DONE")
        upload_status = output_data.get("upload_status", "")
        if final_stage_status == "SKIPPED":
            db_final = "SKIPPED"
        elif stage_id == "STAGE_06_UPLOAD" and upload_status == "PENDING_APPROVAL":
    ...
    def _load_config(self) -> dict:
        """f006_youtube_v4/config.json을 읽어 반환. 실패 시 빈 딕셔너리."""
    ...
    def _get_stage_input(
        ...
        STAGE_01: initial_params 그대로 (주제가 이미 포함됨).
        STAGE_02: STAGE_01 output_data + selected_topic (DB 최신 STAGE_01_INPUT output_data 우선).
        ...
        Returns:
            이 스테이지에 전달할 입력 딕셔너리
        """
        if stage_id == "STAGE_01_INPUT":
            return {**initial_params}

        if stage_id == "STAGE_02_SCRIPT":
            # STAGE_01_INPUT output_data에서 selected_topic 읽기
            # selected_topic이 없으면 None으로 전달 -> validate_input이 REJECTED 처리
            # (RuntimeError를 올리면 job이 FAILED가 되므로 여기서는 raise 금지)
            stage01_output = self._get_stage_output(db_conn, job_id, "STAGE_01_INPUT")
            return {
                **initial_params,
                **stage01_output,
        ...
        if stage_id == "STAGE_04_VIDEO_GEN":
            ...
            return {
                **initial_params,
                # F006는 slides 기반 PPT 렌더링 - scenes는 폴백용으로 함께 전달
                "slides": stage02_output.get("slides", []),
                "scenes": stage02_output.get("scenes", []),  # 폴백 호환성
                "selected_topic": stage02_output.get("selected_topic", ""),
                "generation_backend": initial_params.get("generation_backend", "ppt_slide"),
                "slide_theme": initial_params.get("slide_theme", "dark_blue"),
                # config.json slide_font_path - 사용자 지정 폰트 경로 (빈 문자열이면 기본값)
        ...
        if stage_id == "STAGE_06_UPLOAD":
            ...
            return {
                **initial_params,
                "script_text": stage02_output.get("script_text", ""),
                "selected_topic": stage02_output.get("selected_topic", ""),
