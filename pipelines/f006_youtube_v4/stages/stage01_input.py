# 목적: F006 STAGE_01 - 사용자 직접 입력 + SearXNG 보완 리서치 스테이지.
...
class Stage01Input(BaseStage, BasePipeline):
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F006_STAGE01", "name": "STAGE_01_INPUT"}
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        logger.info(
            f"[F006][STAGE_01][job_id={job_id}] 실행 시작 - "
        ...
        logger.info(
            f"[F006][STAGE_01][job_id={job_id}] SearXNG 보완 검색 시작 - "
        ...
        logger.info(f"[F006][STAGE_01][job_id={job_id}] Ollama enrichment 호출 시작")
        try:
            ...
            logger.info(
                f"[F006][STAGE_01][job_id={job_id}] Ollama enrichment 완료 - "
                f"{len(enriched_context)}자"
            )
        except Exception as e:
            # Ollama 실패 시 fallback: enriched_context = user_context
            logger.warning(
                f"[F006][STAGE_01][job_id={job_id}] Ollama enrichment 실패 "
        ...
        logger.info(
            f"[F006][STAGE_01][job_id={job_id}] 완료 - "
    ...
    def validate_output(self, output: dict) -> ValidationResult:
        ...
        F006는 selected_topic이 항상 자동 확정되므로
        ...
        if brace_start != -1 and brace_end != -1:
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    if parts:
                        logger.info("[F006][STAGE_01] enrichment JSON 파싱 성공")
                        return "\n\n".join(parts)
            except (json.JSONDecodeError, ValueError) as e:
                logger.warning(f"[F006][STAGE_01] enrichment JSON 파싱 실패 ({e}), fallback 사용")

        # 파싱 실패 시 raw_response가 텍스트라면 그대로 사용, 아니면 fallback
        if len(raw_response.strip()) >= 10:
            logger.info("[F006][STAGE_01] enrichment raw 텍스트 사용")
