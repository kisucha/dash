        ...
        if script_text.strip().startswith("{"):
            try:
                ...
                if _parts:
                    ...
                    logger.info(f"[F006][STAGE_03][job_id={job_id}] script_text JSON → narration 추출 ({len(script_text)}자)")
            except Exception:
                pass
            ...
            for attempt in range(300):
                ...
                try:
                    with _urllib_req.urlopen(f"{base_url}/generate/{gen_id}/status", timeout=10) as sr:
                        ...
                        if attempt == 0:
                            logger.info(f"[F006][STAGE_03] VoiceBox status: {str(status_info)[:300]}")
