# 목적: F006 STAGE_02 - 슬라이드 친화적 유튜브 스크립트 생성
...
class Stage02Script(BaseStage, BasePipeline):
    ...
    F006에서는 selected_topic이 STAGE_01에서 자동 확정되므로
    ...
    def get_metadata(self) -> dict:
        ...
        return {"feature_id": "F006_STAGE02", "name": "STAGE_02_SCRIPT"}
    ...
    def validate_input(self, data: dict) -> ValidationResult:
        ...
        F006에서는 STAGE_01이 항상 selected_topic을 자동 확정하므로
    ...
    def execute(self, job_id: int, input_data: dict) -> dict:
        ...
        # F006: enriched_context 우선, 없으면 search_context 사용
        ...
        logger.info(
            f"[F006][STAGE_02][job_id={job_id}] 실행 시작 - "
        ...
        # 1단계: 컨텍스트 섹션 구성
        # ----------------------------------------------------------------
        # F006: enriched_context를 search_context보다 우선 사용
        ...
        logger.info(
            f"[F006][STAGE_02][job_id={job_id}] 슬라이드 스크립트 Ollama 호출 시작 - "
        ...
        try:
            ...
            logger.error(
                f"[F006][STAGE_02][job_id={job_id}] 슬라이드 생성 Ollama 실패: {e}"
            )
        ...
        logger.info(
            f"[F006][STAGE_02][job_id={job_id}] 슬라이드 파싱 완료 - {len(slides)}장"
        )
        ...
        logger.info(
            f"[F006][STAGE_02][job_id={job_id}] 완료 - "
            ...
            try:
                ...
                if isinstance(data, dict) and "slides" in data:
                    ...
                    if isinstance(slides, list) and len(slides) > 0:
                        logger.info(
                            f"[F006][STAGE_02] JSON 파싱 성공 (dict.slides) - {len(slides)}장"
                        )
                        return slides
        ...
        if bracket_start != -1 and bracket_end != -1:
            ...
            try:
                ...
                if isinstance(data_list, list) and len(data_list) > 0:
                    logger.info(
                        f"[F006][STAGE_02] JSON 파싱 성공 (list) - {len(data_list)}장"
                    )
                    return data_list
            except (json.JSONDecodeError, ValueError):
                pass
        ...
        if match:
            try:
                ...
                if isinstance(slides_candidate, list) and len(slides_candidate) > 0:
                    logger.info(
                        f"[F006][STAGE_02] JSON 파싱 성공 (slides 부분 추출) - "
