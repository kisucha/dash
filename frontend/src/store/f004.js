// store/f004.js — F004 유튜브 컨텐츠 제작 V2 파이프라인 Pinia 스토어
...
// F004 스토어 — Composition API 스타일로 정의
export const useF004Store = defineStore('f004', () => {
  // ── 상태 (State) ──
  // jobs: F004 작업 목록 (cursor 기반 페이징)
  const jobs = ref([])
  ...
  async function fetchJobs(limit = 20, status = null) {
    ...
    errorMsg.value = ''
    try {
      const res = await getF004Jobs(limit, null, status)
  ...
  async function fetchMoreJobs(limit = 20) {
    ...
    loading.value = true
    try {
      const res = await getF004Jobs(limit, nextCursor.value, null)
  ...
  async function fetchJob(jobId) {
    loading.value = true
    try {
      const res = await getF004Job(jobId)
  ...
  async function createJob(params) {
    ...
    errorMsg.value = ''
    try {
      const res = await createF004Job(params)
  ...
  async function rerunFromTTS(jobId, scriptText, slides = null) {
    ...
    try {
      ...
      await rerunF004FromTTS(jobId, payload)
      await fetchJob(jobId)
    } catch (e) {
      errorMsg.value = e.response?.data?.detail ?? e.message
      throw e
