...
// ── 현재 작업 편의 참조 ──
const job = computed(() => f005Store.currentJob)
...
async function handleRetry(stageId) {
  ...
  try {
    await f005Store.retryStage(jobId.value, stageId)
  } catch {
...
async function submitReject() {
  ...
  try {
    await f005Store.rejectStage(jobId.value, rejectTargetStageId.value, rejectReason.value.trim())
...
async function handleApprove(finalMeta) {
  ...
  try {
    await f005Store.approveJob(jobId.value, finalMeta)
...
async function handleRerunFromTTS({ scriptText, slides }) {
  ...
  try {
    await f005Store.rerunFromTTS(jobId.value, scriptText, slides)
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    ...
    await f005Store.fetchJob(jobId.value)
...
<template>
  <div class="job-detail-view">
    ...
    <!-- ── 로딩 중 ── -->
    <div v-if="f005Store.loading && !job" class="loading-text">로딩 중...</div>

    <!-- ── 작업 없음 / 오류 ── -->
    <div v-else-if="!job" class="error-box">
      작업을 찾을 수 없습니다.
      <button class="back-btn-inline" @click="router.push({ name: 'F005Feature' })">목록으로</button>
    </div>
    ...
    <template v-else>
      ...
      <div class="page-header">
        <button class="back-btn" @click="router.push({ name: 'F005Feature' })">← F005 목록</button>
