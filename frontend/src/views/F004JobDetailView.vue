<!-- F004JobDetailView.vue — F004 작업 상세 페이지 -->
...
// ── 현재 작업 편의 참조 ──
const job = computed(() => f004Store.currentJob)
...
async function handleRetry(stageId) {
  ...
  try {
    await f004Store.retryStage(jobId.value, stageId)
  } catch {
...
async function submitReject() {
  ...
  try {
    await f004Store.rejectStage(jobId.value, rejectTargetStageId.value, rejectReason.value.trim())
...
async function handleSelectTopic(topic) {
  ...
  try {
    await f004Store.selectTopic(jobId.value, topic.rank, topic.title)
    // selectedStage 갱신
...
async function handleApprove(finalMeta) {
  ...
  try {
    await f004Store.approveJob(jobId.value, finalMeta)
...
async function handleRerunFromTTS({ scriptText, slides }) {
  ...
  try {
    await f004Store.rerunFromTTS(jobId.value, scriptText, slides)
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    ...
    await f004Store.fetchJob(jobId.value)
...
<template>
  <div class="job-detail-view">
    ...
    <!-- ── 로딩 중 ── -->
    <div v-if="f004Store.loading && !job" class="loading-text">로딩 중...</div>

    <!-- ── 작업 없음 / 오류 ── -->
    <div v-else-if="!job" class="error-box">
      작업을 찾을 수 없습니다.
      <button class="back-btn-inline" @click="router.push({ name: 'F004Feature' })">목록으로</button>
    </div>
    ...
    <template v-else>
      ...
      <div class="page-header">
        <button class="back-btn" @click="router.push({ name: 'F004Feature' })">← F004 목록</button>
