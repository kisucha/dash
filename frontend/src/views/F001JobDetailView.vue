<!-- F001JobDetailView.vue — F001 작업 상세 페이지 -->
<!-- StageTimeline + StageResultViewer 2패널 레이아웃 -->
<!-- RUNNING 상태일 때 2초 폴링, PENDING_APPROVAL 상태 배너, 주제 선택/승인 처리 -->
<script setup>
...
// ── 현재 작업 편의 참조 ──
const job = computed(() => f001Store.currentJob)

// ── PENDING_APPROVAL 상태 스테이지가 있는지 확인 ──
const approvalStage = computed(() => {
  if (!job.value?.stages) return null
  return job.value.stages.find(s => s.status === 'PENDING_APPROVAL') ?? null
})
...
async function handleRetry(stageId) {
  ...
  try {
    await f001Store.retryStage(jobId.value, stageId)
  } catch {
...
async function submitReject() {
  ...
  try {
    await f001Store.rejectStage(jobId.value, rejectTargetStageId.value, rejectReason.value.trim())
...
// ── 주제 선택 이벤트 처리 (StageResultViewer emit) ──
async function handleSelectTopic(topic) {
  if (!confirm(`"${topic.title}" 주제를 선택하시겠습니까?`)) return
  try {
    await f001Store.selectTopic(jobId.value, topic.rank, topic.title)
    // selectedStage 갱신
    const updated = job.value?.stages?.find(s => s.stage_id === 'STAGE_01_RESEARCH')
    if (updated) selectedStage.value = updated
  } catch {
    alert('주제 선택에 실패했습니다.')
  }
...
async function handleApprove(finalMeta) {
  ...
  try {
    await f001Store.approveJob(jobId.value, finalMeta)
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    ...
    await f001Store.fetchJob(jobId.value)
...
// onMounted 이후 상태가 바뀔 때 (주제 선택 후 RUNNING 전환 등) 폴링 자동 시작
watch(isRunning, (val) => {
  if (val) startPolling()
  else stopPolling()
})
...
<template>
  <div class="job-detail-view">
    ...
    <!-- ── 로딩 중 ── -->
    <div v-if="f001Store.loading && !job" class="loading-text">로딩 중...</div>

    <!-- ── 작업 없음 / 오류 ── -->
    <div v-else-if="!job" class="error-box">
      작업을 찾을 수 없습니다.
      <button class="back-btn-inline" @click="router.push({ name: 'F001Feature' })">목록으로</button>
    </div>
    ...
    <template v-else>
      ...
      <div class="page-header">
        <button class="back-btn" @click="router.push({ name: 'F001Feature' })">← F001 목록</button>
...
.btn-go-approval:hover { background: #6d28d9; }
