<!-- F001View.vue — F001 유튜브 컨텐츠 제작 파이프라인 메인 페이지 -->
...
async function submitCreateJob() {
  ...
  try {
    const job = await f001Store.createJob(params)
    closeModal()
    // 생성 후 상세 페이지로 이동
    router.push({ name: 'F001JobDetail', params: { jobId: job.id } })
  } catch {
    formError.value = f001Store.errorMsg || '작업 생성에 실패했습니다.'
  }
}
...
async function deleteJob(job) {
  ...
  try {
    await f001Store.deleteJob(job.id)
  } catch {
    alert('삭제에 실패했습니다.')
  }
}
...
<template>
  <div class="f001-view">
    ...
    <div class="page-header">
      ...
      <h1 class="page-title">유튜브 컨텐츠 제작 <span class="badge-f001">F001</span></h1>
      <p class="page-desc">Ollama LLM 파이프라인으로 주제 발굴부터 YouTube 업로드까지 자동화합니다.</p>
    </div>

    <!-- ── 오류 메시지 ── -->
    <div v-if="f001Store.errorMsg" class="error-box">{{ f001Store.errorMsg }}</div>
    ...
    <section class="section">
      ...
      <!-- 로딩 중 -->
      <div v-if="f001Store.loading && f001Store.jobs.length === 0" class="empty-text">
        로딩 중...
      </div>

      <!-- 작업 없음 -->
      <div v-else-if="f001Store.jobs.length === 0" class="empty-text">
        생성된 F001 작업이 없습니다. 새 작업을 생성하세요.
      </div>
      ...
      <div v-if="f001Store.hasMore" class="load-more-wrap">
        <button
          class="btn-load-more"
          :disabled="f001Store.loading"
          @click="f001Store.fetchMoreJobs(20)"
        >
          {{ f001Store.loading ? '로딩 중...' : '더 보기' }}
        </button>
      </div>
    </section>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-box">
        ...
        <div class="modal-header">
          <h2 class="modal-title">새 F001 작업 생성</h2>
...
.modal-step-dot.done::after { background: #86efac; }
...
.modal-step-dot.active .dot-label { color: #4a90d9; font-weight: 600; }
.modal-step-dot.done .dot-label { color: #166534; }
...
.btn-modal-next:hover { background: #357abd; }
...
.btn-modal-create:hover:not(:disabled) { background: #15803d; }
